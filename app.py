...
LOCAL_UNITS={
    ...
    "TWD":{"百万台湾ドル":1.0,"億台湾ドル":0.01,"10億台湾ドル":0.001,"兆台湾ドル":0.000001},
...
def orders_chart(df,company,currency,mode,unit,fx,ptype,n,
    ...
    raw_orders=pd.to_numeric(df["orders"],errors="coerce") if "orders" in df.columns else pd.Series(np.nan,index=df.index,dtype=float)
    raw_backlog=pd.to_numeric(df["backlog"],errors="coerce") if "backlog" in df.columns else pd.Series(np.nan,index=df.index,dtype=float)
    has_orders=raw_orders.notna().any()
    has_backlog=raw_backlog.notna().any()
    ...
    if show_latest and len(df):
        ...
        if has_orders and pd.notna(orders.iloc[-1]):
            xpos=x[-1]-bw/2 if has_backlog else x[-1]
            add_callout(ax,xpos,orders.iloc[-1],fmt(orders.iloc[-1]),orders_color,(-18,30))
        if has_backlog and pd.notna(backlog.iloc[-1]):
            xpos=x[-1]+bw/2 if has_orders else x[-1]
            add_callout(ax,xpos,backlog.iloc[-1],fmt(backlog.iloc[-1]),backlog_color,(30,16))
